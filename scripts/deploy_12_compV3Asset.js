...
async function main() {
	...
	const CompV3 = await CompV3Factory.deploy(assetName, sweep, usdc, cusdc, borrower);
	await CompV3.deployed();

	console.log("CompV3 deployed to: ", CompV3.address);
	console.log(`\nnpx hardhat verify --network ${network.name} ${CompV3.address} "${assetName}" ${sweep} ${usdc} ${cusdc} ${borrower}`);
}
