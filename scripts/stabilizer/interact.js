...
async function main() {
  ...
  async function config() {
    ...
    console.log("   settings enabled:", await stabilizer.settingsEnabled());
  ...
  async function state() {
    ...
    console.log(" minimum_equity_ratio:", pp(await stabilizer.minEquityRatio(), 4), "%");
    console.log("           loan limit:", pp(await stabilizer.loanLimit(), 18), "sweep")
    console.log("       mint available:", (await _getMintAvailable()).toFixed(3), "sweep")
    console.log("       sweep borrowed:", pp(await stabilizer.sweepBorrowed(), 18), "sweep")
    ...
    console.log("           Spread fee:", pp(await stabilizer.spreadFee(), 4), "%");
    console.log("         Spread value:", pp(await stabilizer.accruedFee(), 18), "sweep");
    console.log("         liquidatable:", await stabilizer.liquidatable());
    console.log("  liquidator discount:", pp(await stabilizer.liquidatorDiscount(), 4), "%");
    console.log("           call delay:", pp(await stabilizer.callDelay(), 0), 's');
  ...
  async function _getMintAvailable() {
    const minEquityRatio = Number(pp(await stabilizer.minEquityRatio(), 4));
    const total_value = Number(pp(await stabilizer.getCurrentValue(), 6));
    const sweepBorrowed = await stabilizer.sweepBorrowed();
