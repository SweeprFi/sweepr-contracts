...
async function main() {
    ...
    const amm = await uniswapAMMInstance.deploy(sweep, usdc, sequencer, fee, oracle, frequency, helper, quoter);

    console.log("===========================================");
    console.log(`UniswapAMM Deployed to:${amm.address}`);
    console.log(`\nnpx hardhat verify --network ${network.name} ${amm.address} ${sweep} ${usdc} ${sequencer} ${fee} ${oracle} ${frequency} ${helper} ${quoter}`);
