...
contract("TransferApprover", async function () {
	before(async () => {
		[owner, multisig, user1, user2, user3, user4, lzEndpoint] = await ethers.getSigners();
		...
		await sweep.connect(multisig).addMinter(user1.address, TRANSFER_AMOUNT.mul(3));
		await sweep.connect(user1).minter_mint(user1.address, TRANSFER_AMOUNT.mul(3))
		...
		await sweep.connect(multisig).setTransferApprover(transferApprover.address);
	});
	...
	it('reverts all transfers between non-valid users when validation method is ONLY_VALID', async () => {
		...
		await whitelist.connect(multisig).setUserStatus(user1.address, Const.USER_VALID);
	...
	it('Transfer sweep correctly between valid users when validation method is ONLY_VALID', async () => {
		...
		await whitelist.connect(multisig).setUserStatus(user2.address, Const.USER_VALID);
	...
	it('reverts transfers when validation method is VALID_OR_NEW', async () => {
		...
		await transferApprover.connect(multisig).setValidationType(Const.VALID_OR_NEW)
		...
		await transferApprover.connect(multisig).setTimeDelay(TIME_DELAY);
	...
	it('revert transfers when owner sets user to INVALID', async () => {
		...
		await whitelist.connect(multisig).setUserStatus(user1.address, Const.USER_INVALID);
