...
contract('Compound V3 Asset', async () => {
    before(async () => {
        ...
        await compAsset.configure(
            Const.RATIO, Const.SPREAD_FEE, maxMint, Const.DISCOUNT,
        ...
        await sweep.minter_mint(amm.address, maxMint);
        await sweep.minter_mint(guest.address, maxMint);
    ...
    describe("Main test", async function () {
        ...
        it('mint and sell sweep', async () => {
            ...
            expect(await compAsset.sweep_borrowed()).to.equal(borrowAmount);
        ...
        it('buy and repay sweep', async () => {
            ...
            expect(await compAsset.sweep_borrowed()).to.equal(Const.ZERO);
